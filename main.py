...
def main(message):
    ...
    markup.add(item2)
    item3 = types.KeyboardButton("Leaderboard")
    markup.add(item3)
    item4 = types.KeyboardButton("My name")
    markup.add(item4)
    if users[message.from_user.username][1] == "admin":
        item5 = types.KeyboardButton("Admin's commands")
        markup.add(item5)
...
def message_reply(message):
    ...
    if message.text == "Balance":
        ...
        temp = []
        for i in users:
            temp.append((i, users[i][0]))
        temp.sort(key=lambda x: x[1])
        info = "LEADERBOARD:\n"
        for i in range(len(temp) - 1, max(0, len(temp) - 10), -1):
            info += temp[i][0] + ": " + str(temp[i][1]) + "\n"
        bot.send_message(message.chat.id, info)
        ...
        if wait_send[message.from_user.username][0] == 1:
            try:
                ...
                elif (message.from_user.username in wait_send[message.from_user.username][2] or
                      "@" + message.from_user.username in wait_send[message.from_user.username][2])\
                        and wait_send[message.from_user.username][3] == "send":
                    bot.send_message(message.chat.id, "You cannot send points to yourself")
                else:
